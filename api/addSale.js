...
export default async function handler(req, res) {
  ...
  const { username, addSale } = req.body;

  // Kiểm tra dữ liệu đầu vào
  if (!username || addSale === undefined) {
    console.log('Missing username or addSale');
    return res.status(400).json({ error: 'Username and addSale are required.' });
  }
  ...
  try {
    ...
    const updatedSale = user.sale + addSale;
